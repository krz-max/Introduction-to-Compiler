...
{
  if (!m_formal_parameter_string_is_valid)
  {
    m_formal_parameter_string = "";

    for (const auto &parameter : m_parameters)
    {
      for (const auto &var_node : parameter->getVariables())
      {
        m_formal_parameter_string.append(var_node->getTypeCString()).append(", ");
      }
    }
    ...
    {
      // remove trailing ", "
      m_formal_parameter_string.erase(m_formal_parameter_string.end() - 2,
                                      m_formal_parameter_string.end());
    }

    m_formal_parameter_string_is_valid = true;
  }

  return m_formal_parameter_string.c_str();
}
