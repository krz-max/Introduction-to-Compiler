...
class ForNode final : public AstNode {
  ...
  public:
    ...
    void visitLoopVarDecl(AstNodeVisitor &p_visitor);
    void visitBody(AstNodeVisitor &p_visitor);

    const AssignmentNode *getInitStatement() const { return m_init_stmt.get(); }
    const ExpressionNode *getEndCondition() const { return m_end_condition.get(); }
};
